...
export const OPENINGS: Opening[] = [
    ...
    { eco: 'B30', name: 'Sicilian, Old Sicilian', pgn: '1. e4 c5 2. Nf3 Nc6 3. c3' },
    ...
    { eco: 'D02', name: 'Queen\'s Pawn, London', pgn: '1. d4 d5 2. Nf3' },
    ...
    { eco: 'A05', name: 'Réti, King\'s Indian Attack', pgn: '1. Nf3 Nf6 2. g3' },
    { eco: 'A10', name: 'English Opening', pgn: '1. c4' },
    { eco: 'A13', name: 'English, Closed', pgn: '1. c4 e6' },
    { eco: 'A30', name: 'English, Symmetrical', pgn: '1. c4 c5' },
    ...
    { eco: 'B70', name: 'Sicilian, Dragon', pgn: '1. e4 c5 2. Nf3 d6 3. d4 cxd4 4. Nxd4 Nf6 5. Nc3 g6' },
    { eco: 'C00', name: 'French Defense', pgn: '1. e4 e6 2. d4 d5' },
    { eco: 'C01', name: 'French, Exchange', pgn: '1. e4 e6 2. d4 d5 3. exd5' },
    { eco: 'C02', name: 'French, Advance', pgn: '1. e4 e6 2. d4 d5 3. e5' },
    { eco: 'C11', name: 'French, Classical', pgn: '1. e4 e6 2. d4 d5 3. Nc3 Nf6' },
    { eco: 'C15', name: 'French, Winawer', pgn: '1. e4 e6 2. d4 d5 3. Nc3 Bb4' },
    { eco: 'C19', name: 'French, Winawer, Poisoned Pawn', pgn: '1. e4 e6 2. d4 d5 3. Nc3 Bb4 4. e5 c5 5. a3 Bxc3+ 6. bxc3 Ne7 7. Qg4' },
    { eco: 'D50', name: 'Queen\'s Gambit Declined', pgn: '1. d4 d5 2. c4 e6 3. Nc3 Nf6 4. Bg5' },
    { eco: 'C10', name: 'French, Rubinstein', pgn: '1. e4 e6 2. d4 d5 3. Nc3 dxe4' },
...
export function detectOpening(sanMoves: string[]): Opening | null {
    ...
    const re = /\d+\.(\.\.)?/g;
    const cleaned = sanMoves.join(' ').replace(re, '').trim().split(/\s+/).filter(t => t && !['1-0', '0-1', '1/2-1/2', '*'].includes(t));

    let best: Opening | null = null;
    let bestLen = 0;
    for (const op of OPENINGS) {
        const opRe = /\d+\.(\.\.)?/g;
        const opTokens = op.pgn.replace(opRe, '').trim().split(/\s+/).filter(Boolean);
        let matchLen = 0;
        for (let i = 0; i < Math.min(opTokens.length, cleaned.length); i++) {
            if (opTokens[i] === cleaned[i]) matchLen = i + 1;
            else break;
        }
        if (matchLen > bestLen) { bestLen = matchLen; best = op; }
    }
