...
function enumerateLegalMoves(fen: string): { san: string; uci: string; fenAfter: string }[] {
    const c = createChess(fen);
    return c.moves({ verbose: true }).map((m: any) => ({
        san: m.san,
...
export function detectThreats(fen: string): Threat[] {
    const c = createChess(fen);
    const moves = c.moves({ verbose: true }) as VMove[];
    const threats: Threat[] = [];
    const pieceValue: Record<string, number> = { p: 1, n: 3, b: 3, r: 5, q: 9, k: 0 };

    const captures = moves
        .filter(m => !!m.captured)
        .map(m => ({ m, value: pieceValue[m.captured as string] || 0 }))
